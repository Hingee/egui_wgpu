...
pub struct App {
    ...
    last_size: (u32, u32),
    whiteboard_state: WhiteboardUiState,
}
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &winit::event_loop::ActiveEventLoop) {
        ...
        self.whiteboard_state = WhiteboardUiState {
            tool: Tool::Select,
            zoom: 0.0,
        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                ...
                *last_render_time = now;
                ...
                generate_ui(gui_state, title, &mut self.whiteboard_state);
...
#[derive(Default)]
pub struct WhiteboardUiState {
    pub tool: Tool,
    pub zoom: f32,
    pub pan: egui::Vec2,
}
pub fn generate_ui(
    gui_state: &mut egui_winit::State,
    title: &'static str,
    ui_state: &mut WhiteboardUiState,
) {
    let ctx = gui_state.egui_ctx();

    // =========================
    // Top toolbar (Miro-style)
    // =========================
    egui::TopBottomPanel::top("top_toolbar").show(ctx, |ui| {
        ui.horizontal(|ui| {
            ...
            // Tool buttons
            ui.selectable_value(&mut ui_state.tool, Tool::Select, "Select");
            ui.selectable_value(&mut ui_state.tool, Tool::Pan, "Pan");
            ui.selectable_value(&mut ui_state.tool, Tool::Pen, "Pen");
            ui.selectable_value(&mut ui_state.tool, Tool::Rect, "Rect");
            ui.selectable_value(&mut ui_state.tool, Tool::Text, "Text");

            ui.separator();

            // Zoom controls
            if ui.button("−").clicked() {
                ui_state.zoom = (ui_state.zoom - 0.1).max(0.2);
            }
            ui.label(format!("{:.0}%", ui_state.zoom * 100.0));
            if ui.button("+").clicked() {
                ui_state.zoom = (ui_state.zoom + 0.1).min(4.0);
            }
        });
    });

    // =========================
    // Left panel – Boards / Layers
    // =========================
    egui::SidePanel::left("left_panel")
        ...
        .show(ctx, |ui| {
            ...
            ui.selectable_label(true, "Main Board");
            ui.selectable_label(false, "Ideas");
            ui.selectable_label(false, "Draft");
    ...
    egui::SidePanel::right("right_panel")
        ...
        .show(ctx, |ui| {
            ...
            match ui_state.tool {
                Tool::Select => {
                    ui.label("Select objects to edit");
                }
                Tool::Pen => {
                    ui.label("Pen settings");
                    ui.add(egui::Slider::new(&mut ui_state.zoom, 0.2..=4.0).text("Scale"));
                }
                Tool::Rect => {
                    ui.label("Rectangle settings");
                }
                Tool::Text => {
                    ui.label("Text settings");
                }
                Tool::Pan => {
                    ui.label("Drag to move canvas");
                }
            }
        });
    ...
    egui::TopBottomPanel::bottom("status_bar").show(ctx, |ui| {
        ui.horizontal(|ui| {
            ui.label("LMB: Draw");
            ui.separator();
            ui.label("MMB / Space: Pan");
            ui.separator();
            ui.label(format!("Zoom: {:.2}", ui_state.zoom));
        });
    });
}
